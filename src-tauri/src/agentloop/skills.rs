    ...
    if error.starts_with("storyboard_needs_user_decision:") {
        ...
        if error.contains("requestedMs") || error.contains("user asked for") {
            return json!({
                ...
                "retryable": false,
                "recovery": "Stop this run. Quote the spoken audio length and the user's requested length. Ask which duration to keep. Do not call generate_storyboard again until they answer. The voiced script is saved as the task brief. If the user keeps the spoken length, call generate_storyboard with brief=null and requestedDurationMs=null. targetScriptLength is the script length that fits the requested duration at the speech rate just measured. If the user keeps the requested duration, rewrite the script to about targetScriptLength scriptUnit (within 10%), count it before calling generate_storyboard, and pass the same requestedDurationMs. Do not add only a sentence or two.",

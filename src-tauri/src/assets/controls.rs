...
pub(super) fn task_running(app: &AppHandle, task_id: &str) -> bool {
    open_connection(app).and_then(|connection| connection.query_row(
        "SELECT status = 'running' FROM agent_tasks WHERE id = ?1", [task_id], |row| row.get(0)
    ).map_err(|error| error.to_string())).unwrap_or(false)
}

#[tauri::command]
pub fn get_asset_analysis_progress(app: AppHandle, project_id: String, asset_ids: Option<Vec<String>>) -> Result<AssetAnalysisProgress, String> {
    project_analysis_progress(&open_connection(&app)?, &project_id, asset_ids.as_deref())
}

// 取消资产对应的任务；混合批次中的其他素材重新排队，避免影响其他共享素材。
fn cancel_in_scope(connection: &Connection, project_id: &str, asset_ids: Option<&[String]>) -> Result<(usize, Vec<String>), String> {
    ...
    let tasks = statement.query_map([], |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?)))
        .map_err(|error| error.to_string())?.collect::<Result<Vec<_>, _>>().map_err(|error| error.to_string())?;
    drop(statement);
    let mut remaining = HashSet::new();
    for (id, input) in tasks {
        transaction.execute("UPDATE agent_tasks SET status = 'cancelled', updated_at = ?2 WHERE id = ?1", params![id, timestamp]).map_err(|error| error.to_string())?;
        let mut statement = transaction.prepare(&format!(
            "SELECT id FROM assets WHERE id IN (SELECT value FROM json_each(?1, '$.assetIds'))
             AND coalesce(json_extract(metadata_json, '$.analysisCancelled'), 0) = 0
             AND ({ANALYSIS_STATE_SQL}) IN ('queued', 'analyzing')"
        )).map_err(|error| error.to_string())?;
        for row in statement.query_map([input], |row| row.get::<_, String>(0)).map_err(|error| error.to_string())? {
...
#[tauri::command]
pub fn cancel_asset_analysis(app: AppHandle, project_id: String, asset_ids: Option<Vec<String>>) -> Result<usize, String> {
    let (updated, remaining) = cancel_in_scope(&open_connection(&app)?, &project_id, asset_ids.as_deref())?;
...
#[tauri::command]
pub fn resume_asset_analysis(app: AppHandle, project_id: String, asset_ids: Option<Vec<String>>) -> Result<usize, String> {
    ...
    let rows = statement.query_map(params![project_id, asset_ids.map(|ids| serde_json::json!(ids).to_string())], |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?)))
        .map_err(|error| error.to_string())?.collect::<Result<Vec<_>, _>>().map_err(|error| error.to_string())?;
    drop(statement);
    ...
    for (id, status) in &rows {
        ...
        if status == "ready" { visual.push(id.clone()); } else { technical.push(id.clone()); }
    }
    transaction.commit().map_err(|error| error.to_string())?;
    if !technical.is_empty() { super::analysis::request_asset_analysis(&app, &project_id, &technical)?; }
...
#[tauri::command]
pub fn rename_library_asset(app: AppHandle, project_id: String, asset_id: String, name: String) -> Result<(), String> {
    ...
    if updated == 0 { return Err("Selected asset is not available in this project.".to_owned()); }
    let _ = app.emit("assets-changed", &project_id);
    Ok(())
}

#[tauri::command]
pub fn remove_library_assets(app: AppHandle, project_id: String, asset_ids: Vec<String>) -> Result<BatchAssetActionResult, String> {
    let connection = open_connection(&app)?;
    ...
    super::visual::queue_visual_analysis_batch(&app, &remaining)?;
    let _ = app.emit("assets-changed", &project_id);
    Ok(BatchAssetActionResult { requested_count: asset_ids.len(), updated_count, skipped_count: asset_ids.len().saturating_sub(updated_count) })
}
...
mod tests {
    ...
    fn cancellation_scopes_assets_and_preserves_completed_results() {
        ...
        let cancelled_tasks: i64 = connection.query_row("SELECT COUNT(*) FROM agent_tasks WHERE status = 'cancelled'", [], |row| row.get(0)).unwrap();
        assert_eq!(cancelled_tasks, 2);
        let ready: String = connection.query_row("SELECT metadata_json FROM assets WHERE id = 'ready'", [], |row| row.get(0)).unwrap();

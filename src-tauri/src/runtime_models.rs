...
fn snapshot_status(app: &AppHandle, state: &DownloadState) -> RuntimeModelStatus {
    ...
    let message = if all_ready {
        ...
        "本地模型下载失败，可重试。选镜仍可用，但语义/画面加权会降级。".to_owned()
...
fn download_artifact(app: &AppHandle, id: ArtifactId) -> Result<(), String> {
    ...
    let mut request = agent.get(id.url());
    if existing > 0 {
    ...
    let total = parse_total_bytes(response.header("Content-Range"), response.header("Content-Length"), existing, status);
    {
        let mut state = state_lock()
            .lock()
            .map_err(|_| "runtime_model_state_unavailable".to_owned())?;

...
/// Provider 传输错误只投影为稳定安全码；不得把 URL、模型名、
/// 响应正文或底层传输细节写入 Agent 诊断。不再承载自动重试决策，
/// 失败直接透传给上层，错误码仅用于排障提示。
            ...
            match request_builder.send_string(&request.to_string()) {
                ...
                Err(error) => Err(match error {
                    ureq::Error::Status(status, response) => {
                        ...
                        if is_gateway {
                            match status {
                                401 => "FellowCut 登录已失效，请重新登录。".to_owned(),
                                403 => "FellowCut 使用资格不可用，请在账号页检查试用状态。".to_owned(),
                                413 => "本次画面分析数据过大，模型服务无法接收。".to_owned(),
                                _ => format!("FellowCut 模型服务暂时不可用：HTTP {status}"),
                            }
                        } else {
                            format!(
                    ...
                    ureq::Error::Transport(transport) => {
                        if is_gateway {
                            "无法连接 FellowCut 模型服务，请检查网络后重试。".to_owned()
                        } else {
                            format!(
                                "自定义 API 不可用（{}，模型 {}）:网络错误 {}",
                                config.base_url,
...
mod tests {
    ...
    #[test]
    fn model_request_failure_classification_is_safe_stable_and_never_retried() {

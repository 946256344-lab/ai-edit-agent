...
`App.tsx` 在 Tauri 环境中通过 `local-store.ts` 加载项目、剪辑任务、会话、消息和素材。剪辑任务是项目内的创作目标；会话、storyboard、timeline 和 preview 均被限制在该任务内，素材保持项目级复用。自然语言消息先经项目内 Task Resolver 选择已有任务、创建新任务或澄清，目标任务确定后才写入其 conversation；首次消息或导入仍会在需要时创建项目、任务和会话。顶层工作区按 Agent、素材、成果三种互斥模式渲染：`ConversationWorkspace` 在 Agent 模式只展示消息流、执行卡和 composer，在成果模式集中展示唯一 Workflow、storyboard、timeline/审计与 preview；`AssetManagementPanel` 只在素材模式占用完整主工作区。`App.tsx` 保留项目选择、模式编排、Provider 模态与领域状态编排。
...
|  |- 素材：完整宽度的筛选、目录、素材列表、健康与证据 Inspector
...
素材搜索、状态筛选、收藏、标签、集合、批量整理、任务明细和片段检索仍是受限的本地 Agent/诊断能力，但通过独立的“素材”顶层模式提供人工检查与恢复入口，不常驻 Agent 或成果界面。层级目录只使用安全目录键，不向前端返回绝对路径；素材列表展示缩略图、名称、相对目录、时长、媒体/分析信息及派生证据。“全部素材”直接展示当前有界页，不再强制用户先选择文件夹。`list_asset_page` 继续在 SQLite 执行筛选和有界分页。

最多 200 条的批量操作、技术分析重试和视觉分析跳过继续保留作用域校验和操作审计；显式视觉跳过仍优先于在途粗视觉批次结果，避免状态竞态。顶层模式改变只影响展示组合，不扩大工具副作用或数据访问范围。

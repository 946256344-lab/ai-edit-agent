...
NativeToolLoop 是显式 `NATIVE_TOOL_LOOP=true` 的原生工具实验路径。它绕过 Conversation Router 和 JSON decision schema，按 SQLite 时间顺序读取真实 user/assistant 会话消息，不再把历史拼成带说话人标签的 Prompt；默认注册 9 个只读观察工具（`get_edit_status`、`get_asset_health_summary`、`list_assets`、`search_assets`、`search_asset_segments`、`search_music`、`get_storyboard`、`get_timeline`、`get_text_capabilities`），非只读请求只按本轮明确授权提供对应的主链、文本、音乐下载/编辑或 Jianying 工具；`render_preview` 仍仅在用户明确要求且未被“只查看/不要生成”限制时出现。Native 工具目录覆盖 `replace_text_tracks`、`download_music`、`use_online_music`、`replace_music_tracks` 和 `create_jianying_draft`，但每项仍复用原有 `apply_skill` 与领域校验：许可证和下载归音乐 Provider 所有，文字能力矩阵和剪映兼容性由 Rust 分配，Jianying 只创建新 draft，storyboard 的 `needs_confirmation` 仍会阻止后续非观察写操作。它把本轮真实 `user`、`assistant`、`function_call`、`function_call_output` 项组成后续请求。上下文预算只删除旧消息，最新 function_call 与对应 function_call_output 成对保留，即使工具结果本身超过剩余预算也不拆散，避免模型看不到观察结果而重复调用；`store:false` 时不依赖 Provider 服务端状态。Native 最终回复以 assistant 角色保存，Legacy 仍使用 agent 角色和默认路径；结果由 Rust 校验统一 `tool/status` 包络并移除作用域、本地路径字段，最终任务结果保留真实产物，但显示文案采用模型对成功结果或安全错误的总结。若工具已返回但随后的模型总结请求失败，Rust 以失败终态保存不含传输细节的恢复消息，明确已读取观察数据但未生成模型回复，且不会回退为 Legacy 的“受限操作”文案或伪造产物。

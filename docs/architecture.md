...
NativeToolLoop 是显式 `NATIVE_TOOL_LOOP=true` 的原生工具实验路径。它绕过 Conversation Router 和 JSON decision schema，按 SQLite 时间顺序读取真实 user/assistant 会话消息，不再把历史拼成带说话人标签的 Prompt；默认注册 9 个只读观察工具（`get_edit_status`、`get_asset_health_summary`、`list_assets`、`search_assets`、`search_asset_segments`、`search_music`、`get_storyboard`、`get_timeline`、`get_text_capabilities`），根据请求级权限额外提供 `render_preview`，其中预览只在用户明确要求且未被“只查看/不要生成”限制时出现。它把本轮真实 `user`、`assistant`、`function_call`、`function_call_output` 项组成后续请求。上下文预算只删除旧消息，最新 function_call 与对应 function_call_output 成对保留，即使工具结果本身超过剩余预算也不拆散，避免模型看不到观察结果而重复调用；`store:false` 时不依赖 Provider 服务端状态。Native 最终回复以 assistant 角色保存，Legacy 仍使用 agent 角色和默认路径，其他编辑、副作用及确认门不由 NativeToolLoop 接管；观察结果由 Rust 校验统一 `tool/status` 包络并移除作用域、本地路径字段，仍复用既有 `apply_skill`，最终任务结果保留真实 preview 产物，但显示文案采用模型对成功收据或安全错误的总结。
...
维护记录（2026-08-19）：`agentloop/tools.rs` 集中维护 9 个只读原生 Function Tools：get_edit_status、get_asset_health_summary、list_assets、search_assets、search_asset_segments、search_music、get_storyboard、get_timeline、get_text_capabilities。每项使用 strict JSON Schema 与 additionalProperties=false；严格 schema 的所有属性都列入 required，语义可选值使用 nullable 类型并由 Native loop 再次校验长度、范围和枚举。模型不携带 projectId、conversationId 或本地路径；执行仍由既有 apply_skill 负责，写操作工具不进入原生目录。

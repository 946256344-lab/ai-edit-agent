...
分镜完成后，若 storyboard 有旁白、语音 Provider 已配置、且时间线尚无旁白轨，则 **自动合成配音**（`key_message` 与 `full_script` 均适用）：Agent `generate_storyboard` 与前端成果区共用 `auto_synthesize_storyboard_voiceover`（文本优先 beats）。合成失败只提示，不挡预览；`voiceover_longer_than_picture` 写入 `qualityWarnings`；已有旁白轨则跳过。有大段可朗读文案时 Phase1 后纠偏为 `full_script` 并走 audio-first（TTS 文本优先 `spokenScript`，否则照念 brief，不用模型改写的 beat 旁白）；其后自动配音因已有轨而跳过。

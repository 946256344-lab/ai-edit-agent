...
## Phase 4 精修拆批（2026-09-07，Pass A/长窗收窄 2026-09-08）

Pass A 按素材贪心拆批，每批最多约 40 张窗中点帧；Pass B/C 保持每镜固定时间采样密度，将同镜多帧拼成网格后按最多 10 镜一批调用模型，避免单次上百张图触发网关断连；批结果只回写允许的 `orderIndex`。过短导入头窗（&lt;1.2s）向前并入后窗。Pass B 后若窗内帧间距 &gt;1.5s，Pass C 围绕精修子区间再加密收窄（uncertain 仍整窗加密）。Phase 3 关键帧网格按名次跨池轮询分配（上限 36），卡片标 `keyframeGridAttached`。
...
**Storyboard 五阶段生成流程**：Phase 1 由模型把 brief 拆成包含 `id`、`purpose`、`requiredVisual` 的 beats（短 brief 偏 `key_message`/≤15s 短视频；`key_message` 每 beat 产出 ≤24 字 `onScreenText` 屏幕标记且 `narration` 留空，可读性下限硬门（用户明确要求更长时只放开 15s 时长帽）；有大段可朗读文案则纠偏 `full_script` 并可先 TTS 锁定时长）。Phase 2 **仅本地**：硬过滤就绪视频后语义/词面排序，去同与相似后**补位到目标 12**；仅库耗尽且 &lt;2 才 uncovered；`key_message` 写入可读性节奏计划（`SpeechTiming.kind=pacing`）。Phase 3 模型从池中选 **2–3 互异 asset**（看候选关键帧网格；最终序相邻镜不得同 `assetId`，含跨 beat；全序列任一 `assetId` 不得超过约 40% 上限，prompt 给出 2 镜/3 镜情形的数值；池不足时优先将最弱 beat 标 uncovered）；选片后再按最终 covered 重建节奏计划。Phase 4 **先选内容窗、再段内精修切点**（导入关键帧建窗，不做全片场景扫描；不确定或长窗局部加密；`full_script` 旁白时长托底 / `key_message` 走节奏 fit），禁止换片。Phase 5 Rust `normalize` 自修后硬校验（保留 P1/audio-first 的 `targetDurationMs` 与 `scriptMode`，不改写成镜头总和、不降级模式；`full_script` 仅 lead 回填旁白且只拦画面过长；`key_message` lead 写入 beat 标记、对称校验总时长；交叠在 Phase 4 窗内优先拆开，normalize 不清未挪镜的构图）；精修类失败回 Phase 4，结构/硬上限与 diversity/相邻同片失败不空转 Phase 4。单步重试分离传输/语义预算并带 `previousShots`；耗尽错误含 `partialCandidateSummary`。收尾缺口走 `qualityWarnings` + `insert_clips`，禁止为补镜改 brief 重开。实现位于 `src-tauri/src/storyboard/phases.rs` 与 `step_retry.rs` / `provider_trace.rs`，主流程位于 `storyboard.rs::generate_storyboard_internal`。

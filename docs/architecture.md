...
维护记录（2026-08-19）：新增 `agentloop/tools.rs` 集中维护首批只读原生 Function Tools：get_asset_health_summary、list_assets、get_timeline。每项使用 strict JSON Schema 与 additionalProperties=false；严格 schema 的所有属性都列入 required，get_timeline 的语义可选 timelineVersionId 使用 nullable 类型并允许 null。模型不携带 projectId、conversationId 或本地路径。定义暂不接入用户请求，执行仍由既有 apply_skill 负责。

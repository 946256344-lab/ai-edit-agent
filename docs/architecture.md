...
`useAssetAnalysisController` 与 `AssetAnalysisModal` 跟踪本次导入，提供按实际速度估算的剩余时间、后台分析和取消/继续。`useAssetLibraryEditController` 管理当前列表批量选择、重命名和移除确认；目录、状态筛选及项目切换清空选择。取消素材分析后，剪辑等待仍需继续分析或明确选择仅使用已分析视频。

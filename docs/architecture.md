...
## Phase 4 精修拆批（2026-09-07，Pass A 补齐 2026-09-08）

Pass A 按素材贪心拆批，每批最多约 40 张窗中点帧；Pass B/C 保持每镜固定时间采样密度，将同镜多帧拼成网格后按最多 10 镜一批调用模型，避免单次上百张图触发网关断连；批结果只回写允许的 `orderIndex`。过短导入头窗（&lt;1.2s）向前并入后窗。Phase 3 关键帧网格按名次跨池轮询分配（上限 36），卡片标 `keyframeGridAttached`。

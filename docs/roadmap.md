...
- 多 Agent 共用的单一协作流程、Cursor/Claude Code/OpenCode 薄入口；默认直推 `master`，并行任务才开分支。Windows CI 仍待配置。
...
- 旧素材文件树已恢复为后端权威安全投影：真实项目的 891 条素材无需重导入或重分析即可显示 1 个导入根、10 个一级和 2 个二级目录；目录筛选只返回直属素材，目录键和相对路径不含盘符。素材前端已重建为最小目录浏览器：恢复真实展开/折叠，只保留导入、直属素材、证据和异常恢复，删除传统素材管理控件；后端能力与用户数据不变。
...
- 素材工作区后续只允许从真实 Agent 工作流缺口反推新增界面；不得恢复传统搜索/筛选/收藏/标签/集合/批量任务中心，也不得再通过分页数据重建项目目录事实。
...
- 已完成 `agentloop/policy.rs` 第一阶段纯迁移：工具白名单、负向约束、目标和完成门已与副作用执行隔离。下一步提取 `assets/library.rs` 的安全目录投影和只读查询，再返回 `agentloop/router/state`；worker、事务和技能派发最后移动。每次只迁移一个边界并保留原测试/fixture。

### 2. 生产化媒体与模型接入

- 本地选镜 ONNX（BGE/CLIP）改为瘦安装包 + 应用内后台下载（进行中，见 `docs/changes/2026-09-17-runtime-model-download.md`）。

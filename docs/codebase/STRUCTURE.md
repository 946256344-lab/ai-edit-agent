...
| `.harness/` | 机器可读架构预算与文档同步策略 | `.harness/architecture-budgets.json` |
...
1. `src-tauri/src/lib.rs`：总模块和命令面。
2. `src/App.tsx`：前端如何选择项目、任务和工作区。
3. `src/lib/local-store.ts`：前后端方法映射。
4. `taskrouter.rs` → `agent.rs` → `agentloop.rs`：一次自然语言请求。
5. `assets.rs` → `storyboard.rs` → `timeline.rs` → `preview.rs` → `jianying.rs`：一次产物链路。
6. `db.rs` 与 `models.rs`：持久化事实和序列化形状。

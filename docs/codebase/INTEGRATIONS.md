...
| Tesseract | 安装包运行时，系统安装回退 | 英文 OCR | 无 | 中 | `process.rs`、`assets/analysis.rs`、`scripts/fetch-tesseract.ps1` |
...
- NativeToolLoop 每轮 Provider input 都携带最多 1200 字符的 SQLite 权威状态快照；非观察写工具成功后在下一次请求前重读并替换。完整 payload 超过 40K token 时自主压缩到 30K 内，60K 硬上限不移除该块。

...
- 后果：无需数据库迁移、重新导入或重新分析即可恢复现有素材层级；891 条素材、用户元数据、分析证据和剪辑产物均不改变。空文件夹、只含不支持媒体的文件夹，以及无法从多个素材确定共同结构的单条旧记录不能恢复。素材管理控件删减是后续独立决策，不属于本次目录事实修复。
...
- 状态：展示结论已被 ADR-054 取代；安全与数据边界继续有效

...
- 状态：已实现（2026-08-18）
- 决策：关键帧提取从场景检测（前 30 秒，最多 6 帧）改为固定时间采样（整个视频，精确 4 帧）。采样点：第 1 秒、1/3 处、2/3 处、最后 1 秒（≥1.5s）；短视频（≤2 秒）回退为开头和中间各一帧。提取后的 4 帧通过 `image` crate 拼接为 2×2 网格图（640×360 JPEG），保存为 `<asset_id>_grid.jpg`，路径记录在 `TechnicalMetadata.keyframe_grid_path`。`build_multimodal_content` 函数读取网格图并 base64 编码为 image block，配合元数据 text block 供模型多模态输入。
- 原因：场景检测只覆盖前 30 秒，对长视频（如 5 分钟素材）无法提供尾部内容的视觉证据；场景切换时间点不可控，无法保证均匀覆盖。固定时间采样保证全视频覆盖，采样点可预测，适合批量视觉语义匹配。2×2 网格图既能在单次 API 调用中传递 4 帧信息，又控制了总像素量（640×360 vs 4×320×180 独立图像）。
- 后果：每个素材导入时额外生成一次网格图（4 帧加载 + Lanczos3 缩放 + 拼接 + JPEG 编码），预计增加 50-100ms 延迟和 30-50KB 磁盘占用。旧素材记录的 `keyframe_grid_path` 为 `None`，不影响现有功能。网格生成失败只记录警告，不阻塞素材导入。FFmpeg 使用 `-ss` 直接定位时间点，避免全视频扫描。`SceneSegment` 仍保留用于时间范围元数据，但不再由场景检测生成（`generate_video_keyframes` 返回空 `Vec<SceneSegment>`）。
...
- 状态：已实现
- 决策：根 `AGENTS.md` 只保留全局产品/安全规则、上下文路由和完成门；`src/AGENTS.md` 与 `src-tauri/src/AGENTS.md` 分别拥有 React 和 Rust 的就近约束。`TASKS.md` 使用有界 `ACTIVE_TASKS` 窗口区分当前授权工作与历史记录，`docs/codebase/` 固定为七份有证据的当前源码地图。`.harness/agent-context.json` 声明这些文件、范围、验证命令和跨层允许列表；新的 Node 检查在工作区及 pre-commit 暂存区核对目录完整性、前端 IPC/Rust 注册/API 文档、外部进程/凭据/HTTP 所有权，以及 Rust/TypeScript/fixture 三份 Agent 工具目录。
...
- 决策：素材顶层模式只保留导入文件/文件夹、整体分析摘要、后端权威目录树、当前目录直属素材、派生证据 Inspector、显式源健康检查和保留分析证据的两阶段重链路。删除人工搜索、类型/状态/用户/集合筛选、批量选择、收藏/评分/备注/标签/集合、分析批量动作和素材任务中心。后端数据、命令、Agent 工具和审计契约不删除。目录树以局部 `expandedFolderIds` 保存开合状态，由单一 `toggleAssetFolder` 动作切换；安全导入根首次出现时自动展开，子目录默认折叠，子树只在展开时渲染，`aria-expanded` 必须表达真实状态，素材轮询不得重置用户选择。
- 原因：ADR-051 已明确人工素材管理边界，但后续在一个组件中叠加 50 个 props、约 20 个 `App.tsx` 素材状态和多组人工管理控件，同时在大检查点中丢失了原有开合状态。目录数据恢复后继续修补该组件只会保留相互竞争的状态源；最小目录浏览器更符合“构建 Agent，而不是传统剪辑器”的产品规则。
- 后果：`AssetManagementPanel` 只接收 `model/actions` 两个领域入口并组合四个单一职责组件；`AssetDirectoryTree` 独立拥有开合状态，`App.tsx` 只保留分页/目录选择、健康、重链路和证据所需状态。重链路 UI 固定保留既有分析证据，避免恢复位置时意外触发重新分析。被移除的 UI 功能仍可由后端受限能力、Agent 或未来经独立产品决策重新提供；本次不迁移 schema、不删除用户资料、不重新分析素材，也不改变 storyboard、timeline、preview 或 Jianying draft。

## ADR-058：素材文件树由完整项目的安全目录投影负责

- 状态：已实现；真实 Tauri 目录与直属素材浏览已验证
- 决策：`list_asset_page` 以 `directoryKey` 作为目录筛选输入，并返回每个素材的安全直属目录、完整项目的 `{ key, name, parentKey, directAssetCount }` 目录节点和 `unfiledCount`。目录节点与计数来自完整项目，而不是当前分页；前端只建立父子关系并直接展示后端页，不再以 `folderName` 二次过滤，切换目录时先清空旧页。新文件夹导入继续保存根引用；旧记录若根引用为空或无法形成安全名称，先剥离并按普通/扩展盘符或 UNC server/share 分组。单组有共同父目录时以最末共同级为根，否则以固定“导入素材”为根；多组全部放入独立“导入素材 N”命名空间，确保公开 key 唯一。卷标识不得进入公开键；无法安全解析的记录只让自身归为未分类，不拖累其他安全记录。
- 原因：真实项目 891 条素材均保留源引用，但旧 `folder_reference` 全部指向没有可展示 `file_name` 的 UNC share 根。严格 `Path::file_name` 因此拒绝全部目录，UI 显示 891 条“未归类”；即使后端按完整安全目录筛选，前端又把只有根名的 `folderName` 与完整目录键比较，子目录会被过滤为空。树计数还来自当前最多 100 条素材，无法代表整个项目。
- 后果：无需数据库迁移、重新导入或重新分析即可恢复现有素材层级；891 条素材、用户元数据、分析证据和剪辑产物均不改变。空文件夹、只含不支持媒体的文件夹，以及无法从多个素材确定共同结构的单条旧记录不能恢复。素材管理控件随后由 ADR-059 独立删减，目录事实契约保持不变。
...
- 决策：应用主区域一次只渲染 Agent、素材或成果中的一种。Agent 模式只保留消息、执行卡与 composer；素材模式让 `AssetManagementPanel` 占用完整主工作区；成果模式集中展示唯一 Workflow、storyboard、timeline/审计与 preview。素材和成果不得作为长对话后的附加内容，也不得同时塞进固定窄侧栏。“全部素材”直接显示已加载的有界页。
- 原因：真实桌面审计发现组件拆分破坏了原有互斥渲染：点击 storyboard 仍需越过完整对话与 composer，素材三栏工作台被压进约 330px，重复 Workflow 又让状态与动作竞争。代码能通过构建，但核心产物无法被直接找到。
- 后果：这是纯前端组合与样式决策，不改变 Tauri API、Agent 工具、SQLite、媒体处理或副作用边界。它取代 ADR-051 关于“素材管理不作为主工作区”的展示结论；ADR-051 的 Agent 优先、路径脱敏、作用域校验和素材不直接改写源媒体原则继续有效。后续 UI 结构重构必须验证模式互斥、viewport 无全局溢出和真实桌面截图。
...
# ADR-051：素材管理是 Agent 后台能力，不是传统人工管理器

- 状态：工作区位置被 ADR-054 取代；Agent-first 前端边界由 ADR-059 重申
...
- 状态：已采用（2026-08-15）
- 决策：路由验证失败时使用 validate-then-correct 模式——将错误原因作为纠偏提示反馈给模型后重试一次，而非直接 fail-closed。
- 原因：模型输出偶尔在格式上不符合严格 schema，但语义正确；给模型一次修正机会可大幅降低无效 fail-closed 比率，同时保持硬门（纠偏后仍验证失败则 fail-closed）。
- 后果：每次验证失败最多额外一次模型调用；错误字符串不含用户内容或敏感数据。

## ADR-065：移除静默 fallback，错误路径必须输出真实原因

- 状态：已采用（2026-08-16）
- 决策：后端 Rust 所有"遇错返回硬编码合成值"的静默 fallback 全部替换为显式失败或诚实降级；所有 `log::warn!` 必须包含真实错误变量 `{error}` 或 `{e}`，不得只输出固定字符串。
- 原因：静默 fallback 使 DB 不可用与真实业务失败在日志和状态上无法区分；运营和调试时无法判断系统是正常降级还是静默掩盖了严重错误。
- 后果：降级路径日志信息量增加；所有降级仍封闭失败，不伪造成功结果；公开命令签名和 schema 不变。

## ADR-066：agentloop.rs 分层拆分为四个子模块

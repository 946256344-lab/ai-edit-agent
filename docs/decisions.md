...
UI「剪辑会话」就是剪辑任务（editing task），与 conversation 一一对应。故事版、时间线、预览和编辑器交付只属于所在会话；状态快照、`get_edit_status` 和产物面板都只读当前会话。版本号同样按会话编号，新会话的第一版故事版和时间线都是 v1，局部编辑的派生版本也按这个规则。项目级只共享素材、按内容哈希复用的预览中间缓存，以及选镜时按素材使用次数降权。数据库只追加 `task_version_number` 列，不重建表、不回填：旧版本保留原来的项目内编号，旧会话的新版本从该会话最大号往后接。取代原先「会话只是对话容器、同一任务多会话共享产物」的描述。

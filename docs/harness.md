...
维护记录（2026-08-25）：NativeToolLoop 权威状态快照不扩大工具、IPC、持久化 schema 或可信外部边界；`agent:check` 继续约束 Credential Manager 访问只能留在既有所有者，`harness:check` 继续要求 Native 上下文行为同步架构/API/任务/变更记录并完成独立审查。见 docs/changes/2026-08-24-native-state-snapshot.md。
维护记录（2026-08-27）：Rust dead-code 清理仅删除未使用实现与遗留入口；架构预算、agent 契约与 doc-sync 规则不变。见 docs/changes/2026-08-27-cleanup-rust-warnings.md。

...
- Phase 2 按已实际选中的镜头数动态应用最终 40% 素材占比上限，因此 uncovered beat 不会造成预选与最终校验口径漂移；上一镜头使用的素材直接从下一 beat 候选排除，达到当前镜头规模允许次数的素材也不再进入 Top-12。
- Phase 3 必须按 Phase 2 已有 rough shot（已覆盖 beat）的顺序一对一返回镜头，保持 uncovered beat 不变，不得拆分、合并、增删或重排，避免在素材已锁定后重新制造连续重复。

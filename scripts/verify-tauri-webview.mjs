...
assert.ok(target, 'Voycut WebView target was not found.')
...
await clickButton('.workspace-tabs', '粗剪工作台')
...
assert.equal(await evaluate(`document.querySelector('dialog[open] h2')?.innerText`), '连接 Agent 模型')
await evaluate(`document.querySelector('dialog[open] [aria-label="关闭"]').click()`)
